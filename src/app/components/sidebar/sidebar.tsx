...
import { Archive, BrainCircuit, ChevronDown, ChevronRight, ImagePlay, Images, Layers, MessagesSquare, Moon, PanelLeft, Play, Plus, PlusCircle, ScrollText, Search, Image as Limage, EllipsisVertical, MessageSquare, PenLine, Rocket, SquarePen } from "lucide-react"
            ...
            {isSidebarOpen && <div className="sidebar" onClick={() => { setActiveMenuChatId('') }} ref={SidebarRef}>
                ...
                <div className="sidebar-2-bottom-cont" onClick={(e) => { e.stopPropagation() }}>
                    ...
                    {user && <SidebarButton className="sidebar-button account-btn" onClick={() => { setProfileModal(!openProfileModal) }}>
                        ...
                        </div><span>{user?.name}<br /><span className="plan">Free</span></span>
                    </SidebarButton>}
